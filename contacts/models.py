...
class Dublicate(models.Model):
    ...
    def __str__(self):
        return '%s %s %s' % (self.contact_id.firstname, self.contact_id.secondname,
                             self.contact_id.mobile)
...
class ContactPhoto(models.Model):
    ...
    def create_thumbnail(self):
        ...
        # Make random prefix for thumbnail file
        def random_str(n):
            import random
            import string
            output = ''.join(random.choice(string.ascii_lowercase) for i in range(n))
            return output
